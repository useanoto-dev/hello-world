-- Update trigger to fire on 'completed' status instead of 'preparing'
...
DECLARE
  ...
  item_qty INTEGER;
BEGIN
  -- Process when status changes to 'completed' (finalized)
...
DROP TRIGGER IF EXISTS deduct_inventory_on_order_accepted ON orders;
